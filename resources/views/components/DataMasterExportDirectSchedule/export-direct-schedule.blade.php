<div class="flex w-full justify-between gap-1 px-3">
    <div class="flex gap-2 whitespace-nowrap">
        <a
            ...
            href="/dm-export-direct-schedule/create"
        >
            + New
        </a>
        <a
            ...
            href="/dm-export-direct-schedule/import"
        >
...
<div
    x-data="{
        ...
        deleteUrl: 'dm-export-direct-schedule.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table class="table-data table-fixed" id="dm-export-direct-schedule-table">
        <thead>
            <tr>
                ...
                <th class="text-left md:w-[30%]">Actions</th>
            </tr>
        </thead>
        <tbody></tbody>
    </table>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this Export Direct Schedule? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-export-direct-schedule-table').length) {
                const table = $('#dm-export-direct-schedule-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-export-direct-schedule.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-export-direct-schedule-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-export-direct-schedule-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-export-direct-schedule-table colgroup').remove();
                        $(
                            '#dm-export-direct-schedule-table_wrapper #dm-export-direct-schedule-table_processing',
                        ).remove();
