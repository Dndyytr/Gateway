...
<div
    x-data="{
        ...
        deleteUrl: 'dm-type-container.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table class="table-data table-fixed" id="dm-type-container-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Title</th>
                <th class="pr-5 md:w-[23%]">Description</th>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this Type Container? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-type-container-table').length) {
                const table = $('#dm-type-container-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-type-container.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-type-container-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-type-container-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-type-container-table colgroup').remove();
                        $('#dm-type-container-table_wrapper #dm-type-container-table_processing').remove();
