<div class="flex w-full items-center justify-between gap-2 px-3">
    <div class="whitespace-nowrap">
        <button
            class="cursor-pointer rounded-xs bg-white px-2 py-[.2rem] text-[.65rem] font-medium text-stone-700 shadow-[0_0.55px_1.5px_0.5px_rgba(0,0,0,0.4)] transition-all duration-300 ease-in-out active:bg-slate-200 active:shadow-none bp360:text-[.7rem] bp400:text-xs bp575:text-sm xl:text-base 2xl:text-lg"
            href="/list-tracking/create"
            title="Filter"
        >
...
<div
    x-data="{
        ...
        deleteUrl: 'list-tracking.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table
        ...
        class="table-data"
        id="list-tracking-table"
    >
        <thead>
            <tr>
                <th :class="{'w-full' :is1650}" class="pr-5">Do Number</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Shipment No</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Departure Date</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Customer Name</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Origin</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Destination</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Fleet Category</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Fleet Type</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Plate No</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Driver</th>
                <th :class="{'w-[50%]' :is1650}" class="pr-5">Status</th>
                <th :class="{'w-full' :is1650}" class="text-right">Actions</th>
            </tr>
        </thead>
        <tbody></tbody>
    </table>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).ready(function () {
            if ($('#list-tracking-table').length) {
                const table = $('#list-tracking-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('list-tracking.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#list-tracking-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#list-tracking-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#list-tracking-table colgroup').remove();
                        $('#list-tracking-table_wrapper #list-tracking-table_processing').remove();
