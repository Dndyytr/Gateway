<div class="flex w-full items-center justify-between gap-2 px-3">
    <div class="whitespace-nowrap">
        <a
            ...
            href="/dm-depot-container/create"
            title="Create Depot Container"
        >
            + New
        </a>
    </div>
...
<div
    x-data="{
        ...
        deleteUrl: 'dm-depot-container.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table class="table-data table-fixed" id="dm-depot-container-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Depot Name</th>
                <th class="pr-5 md:w-[23%]">City</th>
                <th class="pr-5 md:w-[20%]">Address</th>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this Depot Container? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-depot-container-table').length) {
                const table = $('#dm-depot-container-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-depot-container.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-depot-container-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-depot-container-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-depot-container-table colgroup').remove();
                        $('#dm-depot-container-table_wrapper #dm-depot-container-table_processing').remove();
