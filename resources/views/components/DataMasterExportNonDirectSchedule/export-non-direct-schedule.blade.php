<div class="flex w-full justify-between gap-1 px-3">
    <div class="flex gap-2 whitespace-nowrap">
        <a
            title="Create Export Non"
            ...
            href="/dm-export-non-direct-schedule/create"
        >
            + New
        </a>
        <a
            class="rounded-xs bg-warnaBiruTuaTombol px-[10px] py-[.2rem] text-[.65rem] font-medium text-white transition-all duration-300 ease-in-out hover:bg-slate-600 bp360:text-[.7rem] bp400:text-xs bp575:text-sm xl:text-base 2xl:text-lg"
            href="/dm-export-non-direct-schedule/import"
        >
...
<div
    x-data="{
        ...
        deleteUrl: 'dm-export-non-direct-schedule.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
    ...
    "
    :class="delayedClass"
    class="bpHeight-1 w-full overflow-auto px-3"
>
    <table
        x-data="{
            isMd: false,
        ...
        x-init="
            ...
            window.addEventListener('resize', () => updateSize())
        "
        :class="isMd && !is1650 ? 'md:w-[max-content]' : 'table-fixed'"
        class="table-data"
        id="dm-export-non-direct-schedule-table"
    >
        <thead>
            <tr>
                <th :class="{'w-full' :is1650}" class="pr-5">Destination</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Vessel</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Voyage</th>
                <th :class="{'w-full' :is1650}" class="pr-5">STF/CLS</th>
                <th :class="{'w-full' :is1650}" class="pr-5">ETD Origin</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Ves Connecting</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Voy Connecting</th>
                <th :class="{'w-full' :is1650}" class="pr-5">ETA Destination</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Connecting City</th>
                <th :class="{'w-full' :is1650}" class="pr-5">ETD Destination</th>
                <th :class="{'w-full' :is1650}" class="text-left">Actions</th>
            </tr>
        </thead>
        <tbody></tbody>
    </table>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this Export Non Direct Schedule? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).ready(function () {
            if ($('#dm-export-non-direct-schedule-table').length) {
                const table = $('#dm-export-non-direct-schedule-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-export-non-direct-schedule.data') }}',
                    ...
                    columns: [
                        ...
                        { data: 'stf/cls', name: 'stf/cls' },
                        { data: 'etd_origin', name: 'etd_origin' },
                        { data: 'ves_connecting', name: 'ves_connecting' },
                        { data: 'voy_connecting', name: 'voy_connecting' },
                        { data: 'eta_destination', name: 'eta_destination' },
                        { data: 'connecting_city', name: 'connecting_city' },
                        { data: 'etd_destination', name: 'etd_destination' },
                        {
                            data: 'actions',
                            name: 'actions',
                            orderable: false,
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-export-non-direct-schedule-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-export-non-direct-schedule-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-export-non-direct-schedule-table colgroup').remove();
                        $(
                            '#dm-export-non-direct-schedule-table_wrapper #dm-export-non-direct-schedule-table_processing',
                        ).remove();
