...
>
    <table class="table-data table-fixed" id="dm-port-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Name</th>
                <th class="pr-5 md:w-[23%]">Country</th>
                <th class="pr-5 md:w-[20%]">City</th>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-port-table').length) {
                const table = $('#dm-port-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-port.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-port-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-port-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-port-table colgroup').remove();
                        $('#dm-port-table_wrapper #dm-port-table_processing').remove();
