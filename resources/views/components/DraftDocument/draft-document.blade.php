...
<div
    x-data="{
        showUploadDraft: false,
        draftURL: '',

        openUploadDraft(url) {
            this.draftURL = url
            this.showUploadDraft = true
        },

        closeUploadDraft() {
            this.showUploadDraft = false
            this.draftURL = ''
        },
        isMd: false,
        is1650: false,
...
>
    ...
    >
        <thead>
            <tr>
                ...
                <th :class="{'md:w-full': is1650}" class="pr-5">Download Draft</th>
    ...
    <div
        ...
        class="fixed inset-0 z-99999999 grid h-screen w-full place-items-center bg-[rgba(0,0,0,0.6)] px-2"
    >
        <form
            x-data="{
                draftFile: false,
                checkdraftFile(event) {
                    this.draftFile = event.target.files.length > 0
                },
            }"
            x-show="showUploadDraft"
            x-cloak
        ...
        >
            ...
            >
                <legend>Upload Draft</legend>
                <button
                    type="button"
                    x-on:click="closeUploadDraft"
            ...
            <div class="m-3 flex flex-col gap-2 text-xs bp400:text-sm bp575:text-base xl:text-lg 2xl:text-xl">
                <label for="upload-draft" class="text-stone-700">Upload Draft</label>
                <input
                    type="file"
                    id="upload-draft"
                    name="upload-draft"
                    x-on:change="checkdraftFile"
                    :class="draftFile ? 'outline-warnaBiruTua' : 'outline-transparent'"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            ...
            if ($('#draft-document-table').length) {
                ...
                const table = $('#draft-document-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('draft-document.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#draft-document-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#draft-document-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#draft-document-table colgroup').remove();
                        $('#draft-document-table_wrapper #draft-document-table_processing').remove();
