...
<div class="m-3 rounded-[3px] border-[1.5px] border-stone-300 px-3 py-2 font-medium">
    ...
    >
        <table
            ...
            class="table-data"
            id="manage-shipment-completed-table"
        >
            <thead>
                <tr>
                    <th :class="{'w-full': is2150}" class="pr-5">Shipment Number</th>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            ...
            if ($('#manage-shipment-completed-table').length) {
                ...
                const table = $('#manage-shipment-completed-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('manage-shipment.completed.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#manage-shipment-completed-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#manage-shipment-completed-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#manage-shipment-completed-table colgroup').remove();
                        $(
                            '#manage-shipment-completed-table_wrapper #manage-shipment-completed-table_processing',
                        ).remove();
