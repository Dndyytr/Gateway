...
<div class="m-3 rounded-[3px] border-[1.5px] border-stone-300 px-3 py-2 font-medium">
    ...
    >
        <table
            ...
            class="table-data"
            id="manage-shipment-on-progress-table"
        >
            <thead>
                <tr>
            ...
            >
                ...
                >
                    ...
                    <p class="text-center font-medium text-stone-700">
                        Are you sure you want to cancel this Manage Shipment On Progress? This action cannot be undone.
                    </p>
                </div>
                <div
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            ...
            if ($('#manage-shipment-on-progress-table').length) {
                ...
                const table = $('#manage-shipment-on-progress-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('manage-shipment.on-progress.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#manage-shipment-on-progress-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#manage-shipment-on-progress-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#manage-shipment-on-progress-table colgroup').remove();
                        $(
                            '#manage-shipment-on-progress-table_wrapper #manage-shipment-on-progress-table_processing',
                        ).remove();
