...
<div class="m-3 rounded-[3px] border-[1.5px] border-stone-300 px-3 py-2 font-medium">
    ...
    >
        <table
            ...
            class="table-data"
            id="manage-shipment-rejected-table"
        >
            <thead>
                <tr>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            ...
            if ($('#manage-shipment-rejected-table').length) {
                ...
                const table = $('#manage-shipment-rejected-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('manage-shipment.rejected.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#manage-shipment-rejected-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#manage-shipment-rejected-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#manage-shipment-rejected-table colgroup').remove();
                        $(
                            '#manage-shipment-rejected-table_wrapper #manage-shipment-rejected-table_processing',
                        ).remove();
