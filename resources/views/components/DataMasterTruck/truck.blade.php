...
>
    <table class="table-data table-fixed" id="dm-truck-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Truck Name</th>
                <th class="pr-5 md:w-[23%]">Truck Type</th>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-truck-table').length) {
                const table = $('#dm-truck-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-truck.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-truck-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-truck-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-truck-table colgroup').remove();
                        $('#dm-truck-table_wrapper #dm-truck-table_processing').remove();
                ...
                if ($customSearch.length) {
                    $customSearch.on(
                        'input',
                        debounce(function () {
                            const searchtruck_name = $(this).val() || ''; // Fallback ke string kosong jika undefined
                            table.search(searchtruck_name).draw();
                        }, 300),
                    );
                } else {
