<div class="flex w-full items-center justify-between gap-2 px-3">
    <div class="whitespace-nowrap">
        <a
            ...
            href="/dm-size-container/create"
            title="Create Size Container"
        >
            + New
        </a>
    </div>
...
<div
    x-data="{
        ...
        deleteUrl: 'dm-size-container.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table class="table-data table-fixed" id="dm-size-container-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Value</th>
                <th class="pr-5 md:w-[23%]">Display</th>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this Size Container? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-size-container-table').length) {
                const table = $('#dm-size-container-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-size-container.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-size-container-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-size-container-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-size-container-table colgroup').remove();
                        $('#dm-size-container-table_wrapper #dm-size-container-table_processing').remove();
