<div class="flex w-full items-center justify-between gap-2 px-3">
    <div class="whitespace-nowrap">
        <a
            ...
            href="/ot-open-ticket/create"
            title="Create Open Ticket"
        >
            + New
        </a>
    </div>
...
<div
    x-data="{
        ...
        deleteUrl: 'ot-open-ticket.destroy',
        openDeleteModal(url) {
            this.deleteUrl = url
...
>
    <table
        ...
        class="table-data"
        id="open-ticket-table"
    >
        <thead>
            <tr>
                <th :class="{'w-[35%]': is1650}" class="pr-5">No</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Quote No</th>
                <th :class="{'w-[80%]' :is1650}" class="pr-5">Subject</th>
                <th :class="{'w-[90%]' :is1650}" class="pr-5">Message</th>
                <th :class="{'w-[80%]' :is1650}" class="pr-5">Deparment</th>
                <th :class="{'w-[80%]' :is1650}" class="pr-5">Attachment</th>
                <th :class="{'w-[80%]' :is1650}" class="pr-5">User Email</th>
                <th :class="{'w-[90%]' :is1650}" class="pr-5">Created At</th>
                <th :class="{'w-full' :is1650}" class="text-center">Actions</th>
            </tr>
        </thead>
        <tbody></tbody>
    </table>
        ...
        >
            ...
            >
                ...
                <p class="text-center font-medium text-stone-700">
                    Are you sure you want to delete this List Open Ticket? This action cannot be undone.
                </p>
            </div>
            <div
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#open-ticket-table').length) {
                const table = $('#open-ticket-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('ot-open-ticket.data') }}',
                    ...
                    columns: [
                        ...
                        { data: 'subject', name: 'subject' },
                        {
                            ...
                            createdCell: function (td, cellData, rowData, row, col) {
                                $(td).addClass('truncate'); // Tambahkan class hanya pada <td>
                            },
                        },
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#open-ticket-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#open-ticket-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#open-ticket-table colgroup').remove();
                        $('#open-ticket-table_wrapper #open-ticket-table_processing').remove();
