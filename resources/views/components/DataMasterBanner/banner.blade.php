...
>
    <table class="table-data table-fixed" id="dm-banner-table">
        <thead>
            <tr>
                <th class="pr-5 md:w-[23%]">Title</th>
                <th class="pr-5 md:w-[23%]">Banner</th>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        $(document).ready(function () {
            if ($('#dm-banner-table').length) {
                const table = $('#dm-banner-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-banner.data') }}',
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-banner-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-banner-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-banner-table colgroup').remove();
                        $('#dm-banner-table_wrapper #dm-banner-table_processing').remove();
