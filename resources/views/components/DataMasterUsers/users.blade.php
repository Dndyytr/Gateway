...
>
    ...
    >
        <thead>
            <tr>
                <th :class="{'w-full' :is1650}" class="pr-5">Nama Perusahaan</th>
                <th :class="{'w-[70%]' :is1650}" class="pr-5">Role</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Pic</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Email</th>
                <th :class="{'w-[50%]' :is1650}" class="pr-5">NIB</th>
                <th :class="{'w-full' :is1650}" class="pr-5">File Legalitas (NIB)</th>
                <th :class="{'w-full' :is1650}" class="pr-5">NPWP</th>
                <th :class="{'w-full' :is1650}" class="pr-5">File NPWP</th>
                <th :class="{'w-full' :is1650}" class="pr-5">Created At</th>
...
<script defer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).ready(function () {
            if ($('#dm-users-table').length) {
                const table = $('#dm-users-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url: '{{ route('dm-users.data') }}',
                    ...
                    columns: [
                        ...
                        { data: 'NIB_number', name: 'NIB_number' },
                        { data: 'company_legality_nib', name: 'company_legality_nib' },
                        { data: 'NPWP', name: 'NPWP' },
                        { data: 'file_of_npwp', name: 'file_of_npwp' },
                    ...
                    createdRow: function (row, data, dataIndex) {
                        const headers = $('#dm-users-table thead th')
                            .map(function () {
                    ...
                    drawCallback: function (settings) {
                        ...
                        if (table.rows({ search: 'applied' }).count() === 0) {
                            const $tbody = $('#dm-users-table tbody');
                            $tbody.empty();
                            $tbody.append(
                                $('<tr>').append(
                                    $('<td>')
                    ...
                    initComplete: function () {
                        $('table#dm-users-table colgroup').remove();
                        $('#dm-users-table_wrapper #dm-users-table_processing').remove();
